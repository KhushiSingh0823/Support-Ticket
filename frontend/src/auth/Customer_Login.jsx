...
const Customer_Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Check user role - allow only 'user' role
      if (res.data.user.role !== 'user') {
        alert('Access denied: You are not an admin.');
        return; // stop further login and navigation
      }
        ...
        <div className="relative z-10 p-14 sm:p-20">
          ...
          <form onSubmit={handleLogin} className="space-y-10">
            ...
            <p className="text-center text-gray-500 text-sm">
              ...
              <Link to="/customerSignup" className="text-indigo-600 hover:underline font-medium">Sign up</Link>
            </p>
          </form>
        </div>
